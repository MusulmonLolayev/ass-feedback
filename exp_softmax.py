...
from utils.softmax_utils import (generate_system_features, \
...
def experiment(
    ...
    for i in range(n_systems):
        systems.append(generate_system(x_system[i], n_features, n_labels))
    ...
    for i, model in enumerate(systems):
        ...
        if np.isnan(best_loss):
            ...
            ep_index = np.argmin(history.history['loss'])
            x_system[i, F_NAMES['epoch']] = ep_index
            x_system[i, F_NAMES['tr_acc']] = history.history['accuracy'][ep_index]

            val_loss, val_acc = model.evaluate(X_val, y_val)
            x_system[i, F_NAMES['val_loss']] = val_loss
    ...
    # and save it
    np.save('./file-results/softmax/x_system_backup.npy', x_system_backup)
    np.save('./file-results/softmax/y_system_backup.npy', y_system)
    ...
    # save a new x_system
    np.save('./file-results/softmax/x_system.npy', x_system)
    np.save('./file-results/softmax/y_system.npy', x_system)
    ...
    ass_mean_error = np.zeros((n_systems, n_labels))
    for i, model in enumerate(systems):
        ...
        print(sum(np.abs(y_pred_delta) < 0.5) / (n_train * n_labels))
        print(sum(np.abs(y_pred_delta) > 1) / (n_train * n_labels))
        # Model prediction
        model.evaluate(X_test, y_test)
        y_pred_test = model.predict(X_test, verbose=0)
        y_delta_test = y_test - y_pred_test
    ...
    with open(f'./file-results/softmax/{dataset_name}_exp1.pkl', 'wb') as f:
    ...
    for i, ep in enumerate(eps):
        ...
        best_accs[i] = metrics.accuracy_score(y_test_max, y_best_pred_label)
        corr_best_accs[i] = metrics.accuracy_score(y_test_max, y_corr_best_pred_label)
        worst_accs[i] = metrics.accuracy_score(y_test_max, y_worst_pred_label)
        corr_worst_accs[i] = metrics.accuracy_score(y_test_max, y_corr_worst_pred_label)
        sel_accs[i] = metrics.accuracy_score(y_test_max, y_sel_pred_label)
        corr_sel_accs[i] = metrics.accuracy_score(y_test_max, y_sel_worst_pred_label)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_name', 
                        default='students', 
                        type=str,
                        choices=['students', 'maternal_health', 'mnist', 'mnist_fashion'])
