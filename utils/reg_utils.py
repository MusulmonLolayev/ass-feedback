...
NUM_NEURONS = list(range(10, 500, 5))
...
def simple_assessor_reg(n_features: int, n_system_input) -> tf.keras.Model:
    ...
    ass_system = skip_connection(ass_system, [64, 128, 256, 512], ['elu', 'elu', 'elu', 'elu'])

    combained_layer = tf.keras.layers.Concatenate(axis=1)([ass_instance, ass_system])
    combained_layer = tf.keras.layers.Dense(128, activation='relu')(combained_layer)
    combained_layer = tf.keras.layers.Dense(64, activation='relu')(combained_layer)
    last_layer = tf.keras.layers.Dense(1)(combained_layer)

    ass_model = tf.keras.Model([instance_input, system_input], last_layer)

    ass_model.compile(
        optimizer=tf.keras.optimizers.Adam(learning_rate=1e-2, decay=8e-2),
    ...
    def update(epochs=1):
        ...
        for indx, count in zip(sys_indx, sys_counts):
            if count:
                ...
                aug_count = int(0.5 * count * (2 * val_acc - 1))
                ...
                if any(cond) and aug_count > 0:
                    ...
                    X_aug.append(X[aug_indices])
                    X_ass_aug.append(x_ass_train[aug_indices])
                    err_aug.append(errors[aug_indices])
