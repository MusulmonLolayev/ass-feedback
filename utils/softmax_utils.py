...
class SVMScikit:
    def __init__(self) -> None:
        self.regr = make_pipeline(StandardScaler(), MultiOutputRegressor(SVR(C=5, epsilon=0.15), n_jobs=12))
...
def load_dataset(dataset_name: str = 'students', 
                 seed: int = 42):
    data = np.loadtxt(f'./datasets/softmax/{dataset_name}.csv', 
                      delimiter=',')
    ...
    assert len(np.unique(y)) == len(np.unique(y_train)), \
      "The training set must contain all labels"
    
    n_labels = y_train.max() + 1

    # one-hot encoding labels
    y_tr_encoded = np.zeros((y_train.shape[0], n_labels))
    y_tr_encoded[np.arange(y_train.shape[0]), y_train] = 1

    y_val_encoded = np.zeros((y_val.shape[0], n_labels))
    y_val_encoded[np.arange(y_val.shape[0]), y_val] = 1

    y_ts_encoded = np.zeros((y_test.shape[0], n_labels))
    y_ts_encoded[np.arange(y_test.shape[0]), y_test] = 1

    return X_train, X_val, X_test, y_tr_encoded, y_val_encoded, y_ts_encoded
...
def generate_system(x_system, n_features, n_labels):
...
def simple_assessor(n_features: int, n_system_input, n_labels) -> tf.keras.Model:
    ...
    output_layer = tf.keras.layers.Dense(n_labels, activation='tanh')(x)

    ass_model = tf.keras.Model([instance_input, system_input], output_layer)
...
def plot_regression_exp3(x_system, y_system, a_acc, b_acc):
    ...
    plt.savefig('./image-results/softmax/exp3.svg')
    plt.savefig('./image-results/softmax/exp3.eps')

    plt.close()

def accuracy_correction(y_pred, y_pred_error, ep):
    # the system
    ...
    y_corr_pred_label = np.argmax(y, axis=1)
...
def train_assessor(X, 
    ...
    y_ass_train = np.zeros((n_train, n_labels))
